...
- **Webhook security.** Anyone who discovers the API Gateway URL could post fake Telegram updates. The deploy script now generates a random secret, registers it with Telegram's `setWebhook`, and the Lambda validates the `X-Telegram-Bot-Api-Secret-Token` header (with constant-time comparison), plus an optional chat-ID allowlist since this is a personal assistant.
